...
function preload() {
    ...
    img = loadImage('data/animalcrossing.jpeg');
}
...
function draw() {
    ...
    tint(200, int(backgroundOpacity * 255)); // Apply fade effect to the image
    ...
    let glitchHue = random(0, 45);     // Random hue 0-360
    ...
    for (let i = 0; i < 10; i++) {
        ...
        let r = random(0, 100);     // Random red value
        let g = random(0, 30);     // Random green value
        let b = random(0, 30);     // Random blue value
    ...
    if (frameCount % 120 === 0) {
        ...
        let glitchText = ['SHOVEL', 'NET', 'FISH', 'SELL', 'BELLS'][floor(random(0, 5))];
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let hue = random(30, 40);   // Random color
        let sat = random(80, 200);  // High saturation
        let bri = random(0, 100);    // Low brightness
    ...
    if (frameCount % 3 === 0) {
        ...
        rect(0, yShift + 20, width, 80);
        
        fill(0, 0, 255, 15); // Blue line
        rect(0, yShift - 20, width, 50);
    }
}
