...
function preload() {
  ...
  bgImage = loadImage("data/windowsXP.jpeg");  //change this to reflect YOUR image
  smallImage = loadImage("data/cloud.png");        //change to yours
}
...
function draw() {
  ...
  push();
    ...
    translate(0, 230, 0);
    rotateX(PI); // Rotate flat to lie on the floor
    noStroke(); // No outline for the floor
    fill("green"); // give the floor its colour
    cone(400, 600); // width, height (both in pixels)
  pop();
  ...
  push();
    ...
    translate(0, -300, 0); // Move the box up off the floor and into the scene a bit
    noStroke();
    ...
    fill("red"); // Give the box its colour
    torus(200, 20); // A cube — one argument means all sides are equal length
  ...
  push();
    ...
    noStroke();
    fill("blue");
    sphere(50); // A sphere — the one argument is the radius
  pop();
...
function draw_2() {
  ...
  //imageMode(CENTER); //use this if you need other modes elsewhere
  ...
  // METHOD 4: Images can follow the mouse! Suggestion: use imageMode(CENTER) above
   ...
   image(smallImage, mouseX, mouseY, 200, 150);
