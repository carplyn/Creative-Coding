...
function preload() {
    img = loadImage('data/3481-10.JPEG');
}
...
function draw() {
    ...
    let glitchSat = random(2, 100);    // Random saturation
    let glitchBri = random(200, 10);      // Random brightness
    ...
    if (frameCount % 10 === 0) {
        ...
        let glitchText = ['H A HA H A HA H A', 'GLITCH TV', 'ERROR', 'HA HA HAHA HAAAA', 'G  T TV', 'HAHA HAHHH'][floor(random(0, 5))];
        fill(255, 255, 255);    // text colour white
        textSize(height/2);          // text size big!
        text(glitchText, width / 1, height / 1); // Center position
        
        // Text distortion - offset text for glitch effect
        fill(random, random, random, random(50, 100)); // Red with transparency
        textSize(height/3);
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let hue = random(20, 900);   // Random color
        let sat = random(80, 200);  // High saturation
        let bri = random(100, 200);    // Low brightness
        let alpha = random(1, 150);  // Very transparent
    ...
    for (let i = 0; i < 400; i++) {
        ...
        let size = random(2, 6); // Random size for dots between 0.5 and 3
    ...
    for (let i = 0; i < 20; i++) {
        ...
        let hue = random(300, 1);
        let sat = random(80, 100);
        let bri = random(10, 80);
        let alpha = random(180, 1);
