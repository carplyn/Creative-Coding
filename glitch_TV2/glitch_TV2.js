...
function draw() {
    ...
    if (frameCount % 10 === 0) {
        ...
        let glitchText = ['PAYOUT', 'JACKPOT', 'WINNER', 'BONUS'][floor(random(0, 4))];
        fill(255, 255, 255);    // text colour white
        textSize(height/12);          // text size big!
        text(glitchText, width / 2, height / 7); // Center position
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let hue = random(2, 900);   // Random color
        let sat = random(10, 10);  // High saturation
        let bri = random(100, 200);    // Low brightness
        let alpha = random(500, 500);  // Very transparent
    ...
    for (let i = 0; i < 400; i++) {
        ...
        let size = random(2, 10); // Random size for dots between 0.5 and 3
